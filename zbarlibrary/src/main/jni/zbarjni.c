...
JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_ImageScanner_init(JNIEnv *env, jclass clazz) {
    ImageScanner_peer = (*env)->GetFieldID(env, clazz, "peer", "J");
}

JNIEXPORT jlong JNICALL
Java_com_sscl_zbarlibrary_sourceforge_ImageScanner_create(JNIEnv *env, jobject thiz) {
...
JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_ImageScanner_destroy(JNIEnv *env, jobject thiz, jlong peer) {
    zbar_image_scanner_destroy(PEER_CAST(peer));
    stats.ImageScanner_destroy++;
}

JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_ImageScanner_setConfig(JNIEnv *env, jobject thiz,
                                                             jint symbology, jint config,
                                                             jint value) {
    zbar_image_scanner_set_config(GET_PEER(ImageScanner, thiz),
                                  symbology, config, value);
}

JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_ImageScanner_parseConfig(JNIEnv *env, jobject thiz,
                                                               jstring config) {
    const char *cfgstr = (*env)->GetStringUTFChars(env, config, NULL);
    if (!cfgstr)
        return;
    if (zbar_image_scanner_parse_config(GET_PEER(ImageScanner, thiz), cfgstr))
        throw_exc(env, "java/lang/IllegalArgumentException",
                  "unknown configuration");
}

JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_ImageScanner_enableCache(JNIEnv *env, jobject thiz,
                                                               jboolean enable) {
    zbar_image_scanner_enable_cache(GET_PEER(ImageScanner, thiz), enable);
}

JNIEXPORT jlong JNICALL
Java_com_sscl_zbarlibrary_sourceforge_ImageScanner_getResults(JNIEnv *env, jobject thiz,
                                                              jlong peer) {
    const zbar_symbol_set_t *zsyms =
...
JNIEXPORT jint JNICALL
Java_com_sscl_zbarlibrary_sourceforge_ImageScanner_scanImage(JNIEnv *env, jobject thiz,
                                                             jobject image) {
...
JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_init(JNIEnv *env, jclass clazz) {
    Image_peer = (*env)->GetFieldID(env, clazz, "peer", "J");
    Image_data = (*env)->GetFieldID(env, clazz, "data", "Ljava/lang/Object;");
}

JNIEXPORT jlong JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_create(JNIEnv *env, jobject thiz) {
...
JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_destroy(JNIEnv *env, jobject thiz, jlong peer) {
    zbar_image_ref(PEER_CAST(peer), -1);
    stats.Image_destroy++;
}

JNIEXPORT jlong JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_convert(JNIEnv *env, jobject thiz, jlong peer,
                                                    jstring format) {
...
JNIEXPORT jstring JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_getFormat(JNIEnv *env, jobject thiz) {
...
JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_setFormat(JNIEnv *env, jobject thiz, jstring format) {
    uint32_t fourcc = format_to_fourcc(env, format);
    if (!fourcc)
        return;
    zbar_image_set_format(GET_PEER(Image, thiz), fourcc);
}

JNIEXPORT jint JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_getSequence(JNIEnv *env, jobject thiz) {
    return (zbar_image_get_sequence(GET_PEER(Image, thiz)));
}

JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_setSequence(JNIEnv *env, jobject thiz, jint seq) {
    zbar_image_set_sequence(GET_PEER(Image, thiz), seq);
}

JNIEXPORT jint JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_getWidth(JNIEnv *env, jobject thiz) {
    return (zbar_image_get_width(GET_PEER(Image, thiz)));
}

JNIEXPORT jint JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_getHeight(JNIEnv *env, jobject thiz) {
    return (zbar_image_get_height(GET_PEER(Image, thiz)));
}

JNIEXPORT jintArray JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_getSize(JNIEnv *env, jobject thiz) {
    jintArray size = (*env)->NewIntArray(env, 2);
    if (!size)
        return (NULL);

    unsigned dims[2];
    zbar_image_get_size(GET_PEER(Image, thiz), dims, dims + 1);
...
JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_setSize__II(JNIEnv *env, jobject thiz, jint width,
                                                        jint height) {
    if (width < 0) width = 0;
    if (height < 0) height = 0;
    zbar_image_set_size(GET_PEER(Image, thiz), width, height);
}

JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_setSize___3I(JNIEnv *env, jobject thiz,
                                                         jintArray size) {
    ...
    zbar_image_set_size(GET_PEER(Image, thiz), dims[0], dims[1]);
}

JNIEXPORT jintArray JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_getCrop(JNIEnv *env, jobject thiz) {
    jintArray crop = (*env)->NewIntArray(env, 4);
    if (!crop)
        return (NULL);

    unsigned dims[4];
    zbar_image_get_crop(GET_PEER(Image, thiz), dims, dims + 1,
                        dims + 2, dims + 3);
...
JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_setCrop__IIII(JNIEnv *env, jobject thiz, jint x, jint y,
                                                          jint w, jint h) {
    VALIDATE_CROP(x, w);
    VALIDATE_CROP(y, h);
    zbar_image_set_crop(GET_PEER(Image, thiz), x, y, w, h);
}

JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_setCrop___3I(JNIEnv *env, jobject thiz,
                                                         jintArray crop) {
    ...
    zbar_image_set_crop(GET_PEER(Image, thiz),
                        dims[0], dims[1], dims[2], dims[3]);
}

JNIEXPORT jbyteArray JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_getData(JNIEnv *env, jobject thiz) {
    jobject data = (*env)->GetObjectField(env, thiz, Image_data);
    if (data)
        return (data);

    zbar_image_t *zimg = GET_PEER(Image, thiz);
    ...
    (*env)->SetObjectField(env, thiz, Image_data, data);
    return (data);
}

JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_setData___3B(JNIEnv *env, jobject thiz,
                                                         jbyteArray data) {
    jint *raw = NULL;
    unsigned long rawlen = 0;
    if (data) {
    ...
    Image_setData(env, thiz, data, raw, rawlen, Image_cleanupIntArray);
}

JNIEXPORT void JNICALL
Java_com_sscl_zbarlibrary_sourceforge_Image_setData___3I(JNIEnv *env, jobject thiz,
                                                         jintArray data) {
